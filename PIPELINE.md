...
文章 URL
  ...
  → 4. TTS 合成（doubao_tts.py，自动随机音色）
  → 5. 自动验收：过短(<70%)扩写重合；过长(>100%)加速重合
  → 6. 计算场景时序（PRE_ROLL / INTER_GAP 公式）
  → 7. 写 index.html（GSAP blur crossfade + 音频 clip）
  → 8. npm run check（0 error）
  → 9. npm run render → MP4
```
